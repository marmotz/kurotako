/**
 * Fixture IR builders for the gen-angular tests. Not part of the published
 * surface. `blogSource` is deliberately cyclic (`User.posts` many <-> `Post.author`
 * one) so deep-relation tests exercise the lazy-builder path.
 */
import {
  createSourceIR,
  type IR,
 ...
 * `Post`) and `Post` (one to-one relation back to `User`) — the same shape
 * `gen-zod`'s fixtures use, so the two packages' tests stay comparable.
 */
