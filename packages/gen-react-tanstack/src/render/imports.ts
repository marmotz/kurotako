/**
 * Import-block accumulator shared by `render/*` and `emit/entity.ts`. Collects the
 * module specifiers + identifiers a rendered entity file needs while it is being
 * assembled, so the final `import` block is built once, sorted, and split into
 * value vs. type-only lines (import lines sorted by module specifier, named
 * imports sorted).
 */
