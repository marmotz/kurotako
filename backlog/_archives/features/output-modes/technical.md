...
export interface OutputConfig {
  ...
  generators?: string[]         // NEW (§Multiple outputs) — restrict this destination to a subset of `config.generators`; default = all
}
```

`ResolvedConfig.output: OutputConfig` (singular) becomes
**`ResolvedConfig.outputs: OutputConfig[]`** (§Multiple outputs below) — one `tako
generate` run can now write to several destinations, each with its own mode / location /
generator subset.
...
This step 5b run — over **every** configured generator, unfiltered — is kept exactly as
described above and is not itself looped per output. It plays two roles: (1) it is the
global collision guard (a generator wrongly emitting `<ns>/index.ts` itself is caught
here regardless of which output(s) end up selecting it), and (2) its result, after
banner (5c), becomes `RunResult.files` / `RunResult.artifacts` — the full computed
virtual tree "as if" a single output selected every generator, used by `--emit-ir` and
(later) `drift-guard`. **What actually gets written to each destination is computed
separately, per output, in step 6** — see §Multiple outputs below.

### Step 6 (write) — one writer call per output

`config.outputs` (§Multiple outputs) is a non-empty array; `run.ts` loops over it. For
each `o` in `config.outputs`:

1. **Select generators**: `names = new Set(o.generators ?? order)` (`order` = every
   generator that actually ran, step 3 — the same "omitted → all" default used for
   `generators[].namespaces` in config-system).
2. **Filter**: keep only the entries of the global collision-checked tree (`collected`,
   pre-barrel, pre-banner — end of step 5) whose path's second segment
   (`<ns>/<generatorName>/…`) is in `names`. No re-collision-check needed: a subset of an
   already-collision-free set cannot newly collide.
3. **Re-synthesize this output's barrels**: `synthesizeRootBarrels(filteredFiles,
   artifacts, logger)` (same function as step 5b, called again on the filtered subset —
   full `artifacts` map is safe to pass as-is, since both `synthesizeRootBarrels` and
   `collectPeerDependencies` derive their contributor set from `files` via
   `contributingGenerators`, not from the map's keys). A namespace with none of its
   generators selected for this output gets no barrel and no files — it is simply absent
   from this destination's tree.
4. **Merge + banner**: `mergeTrees([{ generator: '<filtered>', files: filteredFiles },
   { generator: '<synthesized root barrel>', files: outputBarrels }])`, then
   `applyBanner(...)`.
5. **Write**: `selectWriter(o)` (unchanged selection logic — `mode` absent/`'dir'` →
   `directoryWriter`, `'package'` → `packageWriter`, else `UnsupportedOutputModeError`),
   then `writer.write({ files: outputTree, output: o, artifacts, logger })`. `Writer` /
   `WriteInput` / `directoryWriter` / `packageWriter` are **unchanged** — each still
   receives one `OutputConfig` and one file tree per call; the plurality is entirely a
   `run.ts`-level loop, invisible to the writer layer.
6. **Record**: push `{ output: o, files: writtenPaths }` onto `RunResult.written` (new
   field, see §Multiple outputs).

### Step 7 (`afterEmit`) — once per output

Fires once per entry of `RunResult.written` (only on a real write — `opts.write !==
false`), in the same order as `config.outputs`. `AfterEmitContext` **keeps its existing
shape** (`outputDir`, `files`, `logger`) — a hook written for today's single-output
config keeps working unmodified, it just now may run more than once for a
multi-output config. `outputDir` is `o.dir` in mode A / `o.packagesDir` in mode B for
that output; `files` is that output's written source paths (not mode-B `dist/`). In
mode B the hook fires **after** that output's package build + install (still part of
step 6, inside `packageWriter`, before the loop moves to the next output). A thrown
`HookError` aborts the loop — later outputs in `config.outputs` are not written.
...
Product decision: [overview.md §Multiple outputs](overview.md#multiple-outputs-outputs).
One `tako generate` run can write to several destinations at once (e.g. project A gets
`zod` + `angular`, project B gets `zod` only), each picking a subset of the configured
generators. Generators still run exactly once per run (§Step 6 above) — `outputs[]` only
changes what gets **written where**.

### `RunResult` (core `types.ts`)

```ts
export interface RunResult {
  ir: IR
  order: string[]
  files: VirtualFile[]              // unchanged: the full computed tree, every generator, one synthesized barrel set — step 5b/5c's result
  artifacts: Record<string, GeneratorArtifact>   // unchanged: every generator's artifact
  written: { output: OutputConfig; files: string[] }[]   // NEW — one entry per config.outputs[], absolute paths actually written; [] when write: false
}
```

`files` / `artifacts` keep their current meaning (the full, unfiltered computation — see
step 5b/5c above) so `--emit-ir` and a future `drift-guard` keep one canonical view of
"everything `tako` can produce" independent of how many destinations `outputs[]`
declares. `written` is the new, per-destination view of what actually landed on disk.

### `@kurotako/config` changes

- **`types.ts`**: `OutputOption` gains `generators?: string[]`. `TakoConfig.output?:
  OutputOption` becomes **`TakoConfig.outputs: readonly OutputOption[]`** — required
  (overview decision: no implicit single-output default once the field is a list), so no
  `?:` and no default value synthesized by `defineConfig`.
- **`schema.ts`**: `output: v.optional(v.object({ ... }))` becomes
  ```ts
  outputs: v.pipe(
    v.array(v.object({
  ...
  ```
  Same shape as one entry today, pluralised, `v.minLength(1)` mirroring `sources`'
  `v.minEntries(1)` (`schema.ts:40`).
- **`load.ts`**: the existing mode-`'package'` cross-field check (`packagesDir` + `scope`
  + `NPM_SCOPE_RE`, `load.ts:108-132`) runs **per entry** of `config.outputs` instead of
  once. A new cross-field check, run alongside the existing `namespaces` allowlist loop
  (`load.ts:94-106`): for each `outputs[i]`, every name in `entry.generators ?? []` must
  be a key the generator loop already collected in `seenGenerators` → else
  `UnknownGeneratorError(i, name)`. The `resolvedOutput: OutputConfig` construction
  (`load.ts:178-188`) runs per entry into `resolvedOutputs: OutputConfig[]`, each
  absolutizing its own `dir` / `packagesDir` against `rootDir` and carrying its own
  `generators` through unchanged (already-validated generator names, core does not
  re-validate — same trust boundary as `generators[].namespaces` → `filterIR`,
  `filter.ts:14` "a requested namespace absent … is ignored; config validity is
  config-system's job"). `ResolvedConfig.outputs = resolvedOutputs`.
- **`errors.ts`**: new `UnknownGeneratorError`, mirroring `UnknownNamespaceError`
  (`errors.ts:83-95`):
  ```ts
  ...
  ```
- **`template.ts`**: `CONFIG_TEMPLATE`'s `output: { dir: './generated/kurotako' }` becomes
  `outputs: [{ dir: './generated/kurotako' }]` — still one entry, `generators` omitted
  (all generators), matching the common case.

### Core stays lenient, config-system validates

Same trust split as everywhere else in this pipeline: `@kurotako/core`'s `run()` never
validates `config.outputs[].generators` against `config.generators` — an unknown name in
`names` (§Step 6 point 1) simply matches nothing when filtering `collected`, silently
producing an empty destination. `@kurotako/config` is the only place that can reach a
`ResolvedConfig`, and it always validates first (`UnknownGeneratorError`). A hand-built
`ResolvedConfig` in a core-level test can still pass an unknown name — `run()` stays total
by design (mirrors `filterIR`, `filter.ts:16` docblock).

### Alternatives considered

- **Re-run `generate()` per output.** Rejected (overview): one generator would compute
  the same content N times for N outputs that include it, forces per-output IR/dependency
  bookkeeping, and reopens artifact-sharing questions (`dependsOn` reading another
  generator's artifact) the current single-pass DAG already resolves cleanly.
- **Keep `output` singular, add a separate `additionalOutputs?: OutputOption[]`.** Rejected:
  two shapes for the same concept complicates `load.ts` and the writer loop for no
  benefit — pre-1.0, a straight rename is cheaper than a compatibility shim (overview).
  Renaming a config field with no published version and no external users has no observable
  cost besides updating the fixtures.
- **`outputs[].generators` required (no default-to-all).** Considered, not retained: it
  would make the common single-destination case always list every generator by name,
  pure boilerplate for the config-system's existing "omitted → everything" convention
  already used by `generators[].namespaces`.
- **Aggregate `afterEmit` into one call with a multi-destination context.** Rejected
  (overview Q&A): changes `AfterEmitContext`'s shape (breaking for any existing hook,
  even a single-output one), and a hook wanting an aggregate view can trivially build
  one itself by accumulating across the per-output calls it now receives.

### Accepted risks

- **Overlapping destinations are not validated.** Two `outputs[]` entries pointing at the
  same `dir` (mode A) or the same `packagesDir` + `scope` (mode B) both write during one
  run; the second overwrites the first (mode A: unconditional wipe; mode B:
  `guardAndReset`'s generated-marker check passes since both writes carry the marker).
  No new `TakoError` guards this in v1 — same "tako is the exclusive, unconditional owner
  of its target" posture already accepted for a single output
  ([core-pipeline/technical.md §Accepted risks](../core-pipeline/technical.md)); a
  config authoring two outputs at the same location is a config bug the user can spot
  from the (wasted, but harmless) double write.
- **A `HookError` mid-loop stops later outputs.** `afterEmit` throwing on output 1 of 3
  aborts before outputs 2 and 3 are written (§Step 7) — consistent with the rest of
  `run()` being "sequential, fail-fast" (`run.ts:1-5` file docblock), but means a flaky
  hook can leave a multi-output run partially applied. Not mitigated in v1 (no
  best-effort / continue-on-error mode); revisit if this proves painful in practice.
...
`@kurotako/config` (`errors.ts`), extending `TakoError`, `config_*` codes:

| Class | `code` | Thrown when |
|---|---|---|
| `UnknownGeneratorError` (NEW, §Multiple outputs) | `config_unknown_generator` | an `outputs[i].generators` entry names a generator absent from the top-level `generators[]` |
...
### `config-system/technical.md` — `outputs[]` amendment (§Multiple outputs, new task)

- `OutputOption` (§Config shape and `defineConfig`) gains `generators?: string[]`;
  `TakoConfig.output?: OutputOption` becomes `TakoConfig.outputs: readonly OutputOption[]`
  (required, no default synthesized).
- `TakoConfigSchema.output` (§Structural validation) becomes `TakoConfigSchema.outputs:
  v.pipe(v.array(...), v.minLength(1))` — see §Multiple outputs above for the full schema.
- `load.ts` §Loading and building `ResolvedConfig`: step 6 ("Resolve `output`") becomes
  "resolve each entry of `outputs`"; the mode-`'package'` cross-field check
  (`packagesDir`/`scope`/`NPM_SCOPE_RE`) runs per entry; a new cross-field check validates
  `outputs[i].generators` against the generator names collected in step 5
  (`UnknownGeneratorError`).
- New error class `UnknownGeneratorError` (§Errors table).
- `template.ts`'s `CONFIG_TEMPLATE` illustrates `outputs: [{ dir: '...' }]`.
...
- Both mode-A and mode-B YAML snippets' `output: { ... }` become `outputs: [{ ... }]`
  (§Multiple outputs) — still doc-only illustration, actual format is `.ts`.
...
### `outputs[]` amendment — additional tests (§Multiple outputs)

- `run.ts`: two outputs, disjoint `generators` filters (e.g. `['zod']` and
  `['zod', 'angular']`) → each destination's written files match its filter, each root
  barrel lists only its own destination's generators, `RunResult.written` has two entries
  in `config.outputs` order, `RunResult.files`/`artifacts` still cover every generator
  regardless of filtering; `outputs[i].generators` omitted → that destination gets every
  generator; mode A + mode B mixed across two outputs in one run; `afterEmit` called once
  per output, in order, with that output's own `outputDir`/`files`; a `HookError` on
  output 1 stops output 2 from being written; `write: false` → `RunResult.written === []`,
  nothing on disk.
- `@kurotako/config` `schema.ts`: `outputs: []` (empty array) fails `v.minLength(1)`;
  a minimal one-entry `outputs` passes.
- `@kurotako/config` `load.ts`: `outputs[1].generators: ['nope']` with `generators: [{
  use: zodGenerator }]` → `UnknownGeneratorError` naming index `1` and `'nope'`; two
  outputs, one mode `'dir'` one mode `'package'` missing `scope` → `ConfigShapeError` for
  the second entry only; valid two-entry config → `ResolvedConfig.outputs` has two
  entries, each `dir`/`packagesDir` absolutized independently.
- `@kurotako/config` `template.test.ts`: `CONFIG_TEMPLATE` parses under
  `TakoConfigSchema` with its `outputs: [...]` array.
...
The `outputs[]` amendment (§Multiple outputs) — `ResolvedConfig.outputs` / `RunResult.written`
/ the `run.ts` step 6/7 loop (core), `TakoConfig.outputs` / `UnknownGeneratorError` /
